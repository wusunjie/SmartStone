...
/*
 * Stop the timer to generate the tick interrupts.
 */
static void prvStopTimerInterrupt( void ) PRIVILEGED_FUNCTION;
 ...
 * Stop the timer to generate the tick interrupts.
 */
static void prvStopTimerInterrupt( void )
{
...
#include "stm32f4xx.h"
    ...
    void vPortSuppressTicksAndSleep( TickType_t xExpectedIdleTime )
    {
    	...
    	else
    	{
    		...
    		{
    			/* FIXME:
    			1. Configure an interrupt to bring the microcontroller out of its low
            	power state at the time the kernel next needs to execute.  The
            	interrupt must be generated from a source that remains operational
            	when the microcontroller is in a low power state.
            	2. Correct the kernels tick count to account for the time the
